# Agent Session Manager (`ax`) v0.1.0 — Specification Repository
...
> This repository publishes the normative, implementation-ready contract for Agent Session Manager v0.1.0. It specifies behavior; it does not implement `ax`. Publishing the specification does not claim that any future product acceptance matrix has passed. See [SPEC.md §1](SPEC.md#1-conformance-language-and-scope), [§19](SPEC.md#19-ax-implementation-conformance-and-product-release), and [§20](SPEC.md#20-specification-publication-and-governance).
...
The project owner does not require payload encryption at rest. The spec must not claim default snapshot encryption — and this README does not. SSH protects transport. The security boundary remains a trusted project mesh. `mesh.payload_encryption` must be `none` in `v0.1.0`; any other value fails as unsupported. See [SPEC.md §6.3](SPEC.md#63-field-constraints) and [§16.1](SPEC.md#161-trusted-mesh-model).
...
- **Qwen** has no direct `ax-provider-qwen` claim in `v0.1.0`; task-board prompt-mode bundles only — see [§8.2](SPEC.md#82-native-store-contract-matrix).
...
```
.
├── SPEC.md                          # normative v0.1.0 contract (only normative source)
...
├── .research/                       # required public provider evidence for the v0.1.0 specification package (do not weaken)
...
| `python3 --version` | `Python 3.14.4` | JSON/TOML parsing, JCS identity checks, fixture validation |
...
Diagram sources and render are covered by the same single command; see the exact `structurizr-cli`/`plantuml` flags in [CONTRIBUTING.md](CONTRIBUTING.md#diagrams) and [diagrams/README.md](diagrams/README.md). The validator compares generated C4 PlantUML bytes exactly, validates committed SVG bytes against the v0.1.0 SHA-256 ledger, and compares PlantUML's embedded source/version metadata with a fresh render. This keeps source freshness strict while allowing font and Graphviz geometry to vary across supported documentation platforms. Verify links separately:
...
The repository is intended for public release under the **MIT License**, default branch `main`. The initial specification release target is `v0.1.0`; the signing and authorship metadata — author `Ivan Oparin <oparin@me.com>`, SSH key `~/.ssh/ivanopcode`, SSH-signed commit and annotated tag with local signature verification, no AI `Co-Authored-By` trailer, and an explicit human review gate before any stage/commit/tag/push — are normative in [SPEC.md §20](SPEC.md#20-specification-publication-and-governance) and summarized in [CONTRIBUTING.md](CONTRIBUTING.md#signing-release-and-attribution). Automation MUST stop before staging/commit/tag/push and hand the exact reviewed commands to the user for explicit human execution.
