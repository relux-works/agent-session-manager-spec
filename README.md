# Agent Session Manager (`ax`) v0.4.2 — Specification Repository

| Field | Value |
| --- | --- |
| Public command | `ax` |
| Specification release | `v0.4.2` |
...
> This repository publishes the normative, implementation-ready contract for Agent Session Manager v0.4.2. It specifies behavior; it does not implement `ax`. Publishing the specification does not claim that any future product acceptance matrix has passed. See [SPEC.md §1](SPEC.md#1-conformance-language-and-scope), [§19](SPEC.md#19-ax-implementation-conformance-and-product-release), and [§20](SPEC.md#20-specification-publication-and-governance).

> **Errata baseline:** v0.4.2 supersedes v0.4.0 and v0.4.1 for implementation.
> It preserves immutable Directory Node v1 wire semantics, introduces the
> correctly versioned v2 AX platform vocabulary, and makes common-type
> conformance schema/path-directed. Existing release tags are not moved.
...
- [STANDALONE_TO_AX_TRACEABILITY.md](STANDALONE_TO_AX_TRACEABILITY.md) is the non-normative migration index from every standalone cloning and Session Directory section to its AX destination, reuse, addition, or supersession.
...
10. clone durable session evidence across separately versioned native environments into a new AX logical session without transferring source ownership; and
11. browse a converged Session Directory, inspect evidence-backed lineage and freshness, run bounded source-local enrichment, and execute a confirmed content-addressed continuation plan.
...
```shell
ax session clone doctor --from-provider codex --to-provider claude
ax session clone plan SOURCE --from-provider codex --to-provider claude --fidelity maximal-safe --json
ax session clone run SOURCE --from-provider codex --to-provider claude --no-open --idempotency-key migration-2026-08-27 --json
ax session clone verify BUNDLE_OR_RECEIPT --json
ax session clone open TARGET_OR_RECEIPT --json
```

Semantic conversion is isolated behind `urn:ax:protocol:session-adapter` `1.0.0`, served by the same trusted `ax-provider-<id>` executable and bound to its observed executable digest. Provider Protocol remains `2.0.0`: providers own native transaction and live-write semantics, while the companion adapter owns native-to-canonical capture and canonical-to-native projection. This avoids pairwise converters and does not grant adapters arbitrary filesystem access. See [SPEC.md §7.8](SPEC.md#78-companion-session-adapter-protocol).

Support is not inferred from a provider name, a successful probe, or self-minted evidence. A source reader or target writer is enabled only for an exact environment tuple admitted by the signed `compatibility/supported-environment-tuples-v1.json` registry, with matching contract versions, adapter/provider bindings, fixtures, and unexpired non-revoked evidence. Missing, malformed, stale, partially readable, mismatched, or revoked evidence fails closed; local policy may deny more tuples but cannot approve or un-revoke one. See [SPEC.md §13.14.5](SPEC.md#13145-events-state-and-tuple-admission) and [§19.3](SPEC.md#193-provider-acceptance-suites).

## Session Directory and continuation boundary

The v0.4.2 human namespace is `ax sessions`, with the closed leaves `list`, `inspect`, `lineage`, `scan`, `enrich`, `jobs`, `plan`, `continue`, `operation`, `attach`, and `doctor`. Agents use the same typed engine through `ax sessions q`, `ax sessions grep`, and `ax sessions m`; they must not scrape TUI text. Existing `ax list`, `ax status`, and `ax session clone` retain their v0.3 meanings. See [SPEC.md §14.5](SPEC.md#145-session-directory-cli-result-3-query-and-tui).

```shell
ax sessions list
ax sessions inspect SESSION_OR_INSTANCE
ax sessions lineage SESSION_OR_LINEAGE
ax sessions scan
ax sessions plan SESSION_OR_INSTANCE
ax sessions continue PLAN_ID
ax sessions operation OPERATION_ID
ax sessions doctor
```
...
Directory Node Protocol/Request `2.0.0` use the AX platform vocabulary `macos|linux|wsl2|windows`. Implementations keep immutable v1 compatibility (`darwin|linux|windows`) through explicit dual-stack negotiation; they never relabel or coerce one major into the other. Manifest and Response remain `1.0.0`. See [SPEC.md §7.9](SPEC.md#79-companion-directory-node-protocol) and [§17.5](SPEC.md#175-directory-release-compatibility).
...
The project owner does not require payload encryption at rest. The spec must not claim default snapshot encryption — and this README does not. SSH protects transport. The security boundary remains a trusted project mesh. `mesh.payload_encryption` must be `none` in `v0.4.2`; any other value fails as unsupported. See [SPEC.md §6.3](SPEC.md#63-field-constraints) and [§16.1](SPEC.md#161-trusted-mesh-model).
...
- **Qwen** has no direct `ax-provider-qwen` claim in `v0.4.2`; task-board prompt-mode bundles only — see [§8.2](SPEC.md#82-native-store-contract-matrix).
...
```
.
├── SPEC.md                          # normative v0.4.2 contract (only normative source)
...
│   │   ├── session_state.svg
│   │   ├── mesh_deployment.svg
│   │   ├── cloning_components.svg
│   │   ├── cloning_transaction.svg
...
├── .research/                       # retained provider/directory evidence inherited by v0.4.2 (do not weaken)
...
| `python3 --version` | `Python 3.14.7` | JSON/strict-JSONC/TOML parsing, RFC 8785 UTF-16-ordered JCS identity checks, fixture validation |
| `node --version` | `v25.6.1` | Independent JCS identity, fork projection, digest-path verification |
| `task-board --version` | `dev` | Task evidence and board validation in the authoring environment; not required by clean-checkout publication validation |
...
Diagram sources and render are covered by the same single command; see the exact `structurizr-cli`/`plantuml` flags in [CONTRIBUTING.md](CONTRIBUTING.md#diagrams) and [diagrams/README.md](diagrams/README.md). The validator compares generated C4 PlantUML bytes exactly, validates committed SVG bytes against the release SHA-256 ledger, and compares PlantUML's embedded source/version metadata with a fresh render. This keeps source freshness strict while allowing font and Graphviz geometry to vary across supported documentation platforms. Verify links separately:
...
- a companion Session Adapter protocol for canonical capture/projection, signed exact-tuple admission and revocation, immutable clone generation chains, per-item fidelity accounting, target-native read-back, and lineage receipts;
- a companion Directory Node protocol plus immutable observations, batches, lineage, annotations, enrichment receipts, pure continuation plans, and typed query/result contracts, without centralizing native transcripts or adding a second session/workspace authority;
