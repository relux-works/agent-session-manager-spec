...
parent. Background CLI/SSH/daemon processes may contact an existing minimal
Aqua broker but may not create a credential-dependent server. `launchctl
managername` is only a hint: readiness requires both a functional AX sentinel
and a separate provider-auth smoke bound to the exact tmux server generation,
provider build, and macOS version. The socket and auth state never replicate;
logout/reboot without a verified GUI realm parks recovery until GUI login. See
[SPEC.md §4.2](SPEC.md#42-tmux-backend) and [§16.2](SPEC.md#162-mandatory-exclusions).

`ax NAME` may auto-execute only one uniquely safe non-mutating attach/resume
route. Takeover, fork, move, and ambiguity remain pure plans requiring
confirmation. `ax sync --all` converges immutable objects and policy-allowed
projections only; it never changes ownership or launches a runtime.
...
Graceful takeover waits for a provider safe-turn boundary, quiesces input, performs final workspace/provider/board sync, proves destination broker/auth readiness, gracefully stops the old owner, verifies destination materialization, advances the lease epoch, and resumes with the persisted profile. The old host becomes a replica.
...
Force takeover is an explicit recovery action, not a default choice. It proves destination broker/auth readiness before advancing the epoch, does not claim that the unreachable source process stopped, creates a destination runtime only under the committed winning lease, warns about split-brain risk, and preserves both divergent histories. The old process may still exist but is fenced from authoritative syncing/resuming.
