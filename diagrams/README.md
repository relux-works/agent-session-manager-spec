...
This directory contains the architecture diagrams for the Agent Session Manager (ax) v0.1.0, modeling settled architecture as defined in `SPEC.md`.
...
4. Compares generated C4 `.puml` bytes exactly, checks every committed SVG against the v0.1.0 SHA-256 ledger, and compares embedded PlantUML source/version metadata with the fresh render. Font and Graphviz geometry may vary by platform without weakening committed-byte integrity or source freshness.
